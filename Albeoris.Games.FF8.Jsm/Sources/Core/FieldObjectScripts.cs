...
namespace Albeoris.Games.FF8.Jsm.Core
{
    public sealed class FieldObjectScripts
    {
        ...
        private readonly PriorityQueue<Executer, Int32> _executionQueue = new PriorityQueue<Executer, Int32>();
        ...
        {
            while (_executionQueue.TryDequeue(out var executer, out _))
                executer.Complete();
        }
        ...
        private void EnsureInitialized()
        {
            ...
            foreach (var item in _dic.Values)
                _executionQueue.Enqueue(new Executer(item), 0);
        ...
        {
            if (_dic.TryGetValue(scriptId, out var obj))
                return obj;

            throw new ArgumentException($"A script (Id: {scriptId}) isn't exists.", nameof(scriptId));
        }

        private Boolean HasPriority(Int32 priority)
        {
            foreach ((Executer _, Int32 queuedPriority) in _executionQueue.UnorderedItems)
            {
