...
namespace Albeoris.Games.FF8.Jsm.Instructions
{
    ...
    {
        private IReadOnlyList<IJsmExpression> _operands = Array.Empty<IJsmExpression>();

        protected JsmInstruction()
        {
        }

        public IReadOnlyList<IJsmExpression> Operands => _operands;
        ...
        public static JsmInstruction TryMake(Jsm.Opcode opcode, Int32 parameter, IExpressionStack stack)
        {
            if (Factories.TryGetValue(opcode, out var make))
            {
                OperandRecordingStack recordingStack = new OperandRecordingStack(stack);
                JsmInstruction instruction = make(parameter, recordingStack);
                instruction._operands = recordingStack.GetOperands();
                return instruction;
            }
        ...
        private sealed class OperandRecordingStack : IExpressionStack
        {
            private readonly IExpressionStack _stack;
            private readonly List<IJsmExpression> _operands = new List<IJsmExpression>();

            public OperandRecordingStack(IExpressionStack stack)
            {
                _stack = stack;
            }
            ...
            {
                IJsmExpression expression = _stack.Pop();
                _operands.Add(expression);
                return expression;
            }

            public IReadOnlyList<IJsmExpression> GetOperands()
            {
                _operands.Reverse();
        ...
        private delegate JsmInstruction Make(Int32 parameter, IExpressionStack stack);
