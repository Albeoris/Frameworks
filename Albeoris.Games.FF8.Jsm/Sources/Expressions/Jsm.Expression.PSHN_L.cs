using System;
using System.Globalization;
using Albeoris.Games.FF8.Jsm.Core;
using Albeoris.Games.FF8.Jsm.Format;

namespace Albeoris.Games.FF8.Jsm
{
    public static partial class Jsm
    {
        public static partial class Expression
        {
            public sealed class PSHN_L : IConstExpression
            {
                private Int32 _value;

                public Int32 Value
                {
                    get => SourceOperation?.Parameter ?? _value;
                    set
                    {
                        JsmOperation.ValidateParameter(value);
                        _value = value;

                        if (SourceOperation is not null)
                            SourceOperation.Parameter = value;
                    }
                }

                public JsmOperation? SourceOperation { get; private set; }

                Int64 IConstExpression.Value => Value;

                public PSHN_L(Int32 value)
                {
                    JsmOperation.ValidateParameter(value);
                    _value = value;
                }

                internal void Bind(JsmOperation sourceOperation)
                {
                    ArgumentNullException.ThrowIfNull(sourceOperation);
                    if (sourceOperation.Opcode != Jsm.Opcode.PSHN_L)
                        throw new ArgumentException("The source operation must be PSHN_L.", nameof(sourceOperation));

                    SourceOperation = sourceOperation;
                    _value = sourceOperation.Parameter;
                }

                public override String ToString()
                {
                    return $"{nameof(PSHN_L)}({nameof(Value)}: {Value})";
                }

                public void Format(ScriptWriter sw, IScriptFormatterContext formatterContext, IServices services)
                {
                    sw.Append(Value.ToString(CultureInfo.InvariantCulture));
                }

                public IJsmExpression Evaluate(IServices services)
                {
                    return this;
                }

                public ILogicalExpression LogicalInverse()
                {
                    return ValueExpression.Create(Value == 0 ? 1 : 0);
                }

                public Int64 Calculate(IServices services)
                {
                    return Value;
                }
            }
        }
    }
}
